...
from .types import Variable, Function, Data, NodeSpec, GraphObject, local, LazyClass
...
def trace_object(obj, arg_table=None, mapped_names=None):
    ...
    if isinstance(obj, Function):
        var_name = trace_func(obj, arg_table, mapped_names)
    ...
    return var_name, arg_table, mapped_names
...
def expand_nested_graphobjs(in_obj, is_root=True):
    if not _is_supported_container(in_obj):
        return in_obj
